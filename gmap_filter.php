	...
	class FILTERREQUEST{
		...
		public function general(){
			return   "SELECT * FROM(
					  ...
					  WHERE CONCAT_WS(',', Community,Municipality,Ml_num,Zip,Rltr) LIKE '%{$this->query_string}%'
					  {$this->where}
					  UNION
		...
		public function residential_condo_statement(){
			return   "SELECT * FROM(
					  SELECT DISTINCT Addr, Ml_num,Zip, Ad_text,Municipality,County,Area,Timestamp_sql,Lp_dol,Br,Bath_tot, Type_own1_out as type_of_home,
					  	'rets_property_residentialproperty' as prop_type 
					  FROM rets_property_residentialproperty
					  WHERE CONCAT_WS(',', Community,Municipality,Ml_num,Zip,Rltr) LIKE '%{$this->query_string}%'
					  {$this->where}
					  UNION
					  SELECT DISTINCT Addr, Ml_num,Zip, Ad_text,Municipality,County,Area,Timestamp_sql,Lp_dol,Br,Bath_tot, Type_own1_out as type_of_home,
					  	'rets_property_condoproperty' as prop_type
					  FROM rets_property_condoproperty
					  WHERE CONCAT_WS(',', Community,Municipality,Ml_num,Zip,Rltr) LIKE '%{$this->query_string}%'
					  {$this->where}) as property
					  ORDER BY Timestamp_sql DESC";
		...
		public function residential_statement(){
			return   "SELECT DISTINCT Addr, Ml_num,Zip, Ad_text,Municipality,County,Area,Timestamp_sql,Lp_dol,Br,Bath_tot, Type_own1_out as type_of_home,
						'rets_property_residentialproperty' as prop_type 
					  FROM rets_property_residentialproperty
					  WHERE CONCAT_WS(',', Community,Municipality,Ml_num,Zip,Rltr) LIKE '%{$this->query_string}%'
					  {$this->where}
		...
		public function commercial_statement(){
			return   "SELECT DISTINCT Addr, Ml_num,Zip, Ad_text,Municipality,County,Area,Timestamp_sql,Lp_dol,Bath_tot,Prop_type as type_of_home,
